...
库和密钥的上层目录也须受信任：由 root 或库所属账号控制，不能允许其他账号替换其中的目录。工具会检查上层目录的属主及写权限；系统中带 sticky 位的可信临时目录可用，普通共享可写目录会被拒绝。不要通过放宽权限绕过检查。
...
实现中，scope_path 选择项目/环境对应的文件，read 核对解密后的项目/环境字段，bindings 选择要注入的变量；这些检查防止误选或混用数据，**没有建立「调用者只能读取自己项目」的授权机制**。库管理账号能改用另一组 project/environment，读取其有权访问的其他密文。--bind 也不是访问控制列表，不能把它当成对恶意同账号程序的限制。

部署前确定并记录所需的信任边界：

- **需要应用间隔离**：使用 root 管理的库和 identity；不同应用使用不同的非 root 运行账号，通过受信任、由 root 控制的启动器解密后降权，只注入本应用所需值。不能向应用开放任意 sudo、Docker socket、其他应用可写的启动器或共享可读的凭据文件。不同环境若也要求隔离，运行身份和权限也须相应区分。
- **明确接受同账号互信**：可以采用单账号布局，但须在中文部署记录中注明应用共享信任边界；不能描述成已实现项目间凭据隔离。不能仅凭现有应用恰好共用账号就推断用户接受这一限制。
- **要求隔离但当前无法做到**：在部署前阻断，说明共享账号、权限或运行方式的具体限制；不得降级为同账号运行后宣称安全。

在目标服务器核验实际进程 UID/GID 和附加组，并以应用账号测试对 identity、密文库、其他应用敏感目录的访问确实被拒绝；同时核验启动器、unit 及其依赖文件不能被应用账号修改。仅看账号名称、分组字段或 chmod 数字不足以确认有效权限，还要考虑共享组、ACL 和实际目录访问。验证时只输出身份与访问结果，不输出凭据值。

启动器会继承调用进程的环境，再覆盖显式绑定的变量；它不会自动清理环境中已有的其他秘密。启动环境必须只包含该应用需要的配置，不能从加载了其他项目 Key 的管理 shell 直接启动隔离应用。应用本身仍能读取并泄露已经注入给它的凭据。
...
不要用 echo 导入：它通常附加换行。工具保留来源中的全部字符，不能自动判断空白是否误带；入库前核对来源，入库后用实际业务验证，不依赖工具自动 strip。单值上限 64 KiB；整个项目/环境库上限约 4 MiB，不用于证书归档或大文件。
...
部署后检查实际主进程用户为 translator，且该账号读不到 identity/其他项目密文；只检查 unit 的 User= 行会误判。需要隔离的另一个应用必须换用另一个独立账号，不能复制示例后继续共用 translator。一个服务使用一个作用域，--bind 只列出需要的值。非敏感配置继续遵循项目约定，不能在 Environment= 行里复制秘密。

如采用单账号布局，直接以该账号执行 exec 可省略 --as-user；同时必须明确记录同账号应用能访问其凭据库的限制。OS 权限检查不是能力隔离系统，不应宣称这一模式能隔离恶意应用。

回归测试会同时验证「库所属账号能读取其不同项目」和「root 启动器降权后的应用账号读不到受限库」。后者仅覆盖测试夹具中的权限安排，不证明任意生产环境已经正确隔离，也不保证没有其他泄露路径；最终交付须说明实测范围。
...
3. systemd 重启应用；Compose 需重新创建对应容器。已经运行的进程不会自动更新环境变量。
