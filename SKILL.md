...
1. **理解项目与当前状态**：按上述适配规则阅读项目部署说明及实际启动配置，确认运行时、构建/启动命令、健康接口、必要环境变量、数据库迁移、持久化路径和并发需求。先明确可行方案或阻断项，再进行后续线上部署操作。检查目标服务器的登记表、监听端口、Nginx 与进程管理器。检查配置时筛选必要信息，避免把包含秘密的完整 unit、环境或配置输出到工具日志。
...
用中文简要报告实际 URL、部署版本、进程与配置位置、验证结果、持久化/回滚位置，以及两份服务器文档的路径。说明如何参考并适配项目部署文档；被阻断时报告具体冲突和证据，不能写成部署成功。涉及凭据时列出引用、接入方式、实际运行账号及隔离依据，绝不展示值。说明已落实的访问控制和实际剩余事项；尚未验证的外部连通性或服务商 Key 有效性必须标为未验证。CI 通过只说明所列测试在测试环境通过，不能作为已在目标服务器建立权限隔离或完成安全审查的证明。
